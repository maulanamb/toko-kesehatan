...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ▼▼▼ PERUBAHAN SQL (LANGKAH 2) ▼▼▼
    // Ambil data keranjang DAN toko_id
    ...
    try {
        ...
        // --- Langkah 5 (Langkah B): Masukkan setiap item ke 'order_details' ---
        
        // ▼▼▼ PERUBAHAN SQL & LOGIKA (LANGKAH 5) ▼▼▼
        $sql_details = "INSERT INTO order_details (order_id, product_id, quantity, price_at_purchase, status_vendor) 
                          VALUES (?, ?, ?, ?, ?)"; // status_vendor sekarang jadi variabel
        ...
        foreach ($cart_items as $item) {
            ...
            // Logika Auto-Approve untuk Admin
            if (is_null($item['toko_id'])) {
                $status_vendor_item = 'Approved'; // Auto-approve item milik Admin
            } else {
                $status_vendor_item = 'Pending'; // Item milik Vendor perlu persetujuan
            }
            
            // Masukkan ke order_details (tipe data baru "iiids")
            $stmt_details->bind_param("iiids", 
                $order_id, 
                $item['product_id'], 
                $item['quantity'], 
                $item['price'], 
                $status_vendor_item // Gunakan status yang sudah ditentukan
            );
        ...
        // --- Langkah 6: Kosongkan keranjang 'cart_items' ---
