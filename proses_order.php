...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_cart = "SELECT p.product_id, p.price, c.quantity, p.toko_id 
                 FROM cart_items c
    ...
    try {
        ...
        // ▼▼▼ PERBAIKAN DI SINI (Baris 52) ▼▼▼
        // "isss" diubah menjadi "idss" (i=integer, d=double, s=string, s=string)
        $stmt_order->bind_param("idss", $user_id, $total_amount, $payment_method, $shipping_address);
        // ▲▲▲ SELESAI PERBAIKAN ▲▲▲
        ...
        foreach ($cart_items as $item) {
            ...
            $stmt_stok->execute();
            if ($stmt_stok->affected_rows === 0) throw new Exception("Stok tidak mencukupi untuk produk ID: " . $item['product_id']);
        }
        $stmt_details->close();
        $stmt_stok->close(); 
        ...
        die("Error Database: " . $exception->getMessage());
    }
    ...
    if (!empty($user_email) && MAIL_PASSWORD != 'KUNCI_SMTP_DARI_BREVO') {
        ...
        try {
            ...
            $mail->Body    = "Halo " . htmlspecialchars($username) . ",<br><br>Terima kasih atas pesanan Anda! Pesanan Anda dengan nomor <b>#" . $order_id . "</b> sedang kami proses.<br><br>Terlampir adalah invoice untuk pesanan Anda.<br><br>Salam,<br>" . MAIL_SENDER_NAME;
        ...
        //echo "Pengiriman email dilewati karena email user kosong atau config email belum diisi.";
        //exit;
    }
